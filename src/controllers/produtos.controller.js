...
module.exports = {
    ...
    async create(req,res){
        ...
        let product = await Produto.findOne({nome_produto});//para verificas se não existe esse Produto
        if(!product){
            data = {nome_produto,descricao_produto,preco_produto,qtd_produto};
            product = await Produto.create(data);// await e feito para esperar para não criar Produto vazio.
    ...
    async update(req,res){
        const {_id,nome_Produto,email_Produto,senha_Produto,tipo_Produto} = req.body;
